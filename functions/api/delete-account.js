...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (!user) return Response.json({ error: 'Incorrect password.' }, { status: 401, headers });

    await env.DB.prepare('DELETE FROM users WHERE username = ?1 COLLATE NOCASE').bind(username.trim()).run();
